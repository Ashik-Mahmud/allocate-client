    ...
    return (
        <div className="w-full  space-y-6 p-4 text-neutral-900 dark:text-neutral-100 font-sans">
            ...
            <div className="space-y-4">
                ...
                onReplyToUser={post?.allowComments === true ? (username: string) => setCommentText(prev => prev?.length ? `${prev} @${username} ` : `${prev}@${username} `) : undefined}
                />
                ...
                {user?.id !== post?.authorId && post?.allowComments === false ? (
                    <div className="bg-white dark:bg-slate-900/20 border border-slate-200/60 dark:border-slate-800/80 rounded-xl p-6 text-center shadow-xs">
                        <MessageSquareOff className="w-5 h-5 mx-auto mb-2 text-slate-400 dark:text-slate-500 stroke-[1.8]" />
                        <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                            Comments are disabled
                        </p>
                        <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5 max-w-xs mx-auto">
                            The author has turned off responses for this community workspace thread.
                        </p>
                    </div>
                ) : (
                    ...
                    <form onSubmit={handleAddComment} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm space-y-3">
                        {user?.id === post?.authorId && post?.allowComments === false && (
                            <div className="text-xs text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/30 rounded-md p-2 flex items-center gap-2 animate-fade-in">
                                <ShieldX className="w-3.5 h-3.5 stroke-[2.5] shrink-0" />
                                <span>Comments are currently disabled by you. Others cannot reply, but you can still post updates.</span>
                            </div>
                        )}
                        ...
                        <div className="relative">
                            ...
                            {commentText.trim() && (
                                <button
                                    ...
                                    disabled={isRefineNoteLoading || !commentText.trim() || user?.organization?.plan_type === PlanType.FREE || true}
                                ...
                                >
                                    ...
                                    <span>{isRefineNoteLoading ? "Refining..." : "Refine with AI"}</span>
                                </button>
                            )}
                        </div>
